...
import static prefuse.util.GraphLib.LABEL;
import static prefuse.util.GraphLib.LABEL_SCHEMA;
...
public class DataMountain extends Display {
    ...
    public DataMountain(Table t) {
        ...
        /*m_vis.addTable("data", t);
         ...
         t.addColumn("XPTO", int.class);
         for (int i = 0; i < 6; i++)
         t.getTuple(i).set("XPTO", i);
  
        
         JPrefuseTable.showTableWindow(edges);
         ...
         int i = 0;
         for (Iterator<Node> it = nodes; it.hasNext(); i++) {
         Node n = it.next();
         ...
         try {
         if (i % 2 != 0) {
         graph.addEdge(graph.getNode(i - 1), graph.getNode(1));
         } else {
         if ((i > 0) && (i % 2 == 0)) {
         graph.addEdge(graph.getNode(i - 2), graph.getNode(1));
         }
         }
         } catch (Exception e) {
         System.out.println("Não conseguiu criar as arestas. Erro: " + e.getMessage());
         }
         }*/
        ...
        nodeRenderer.setMaxImageDimensions(150, 150);
    ...
    public static JComponent demo() {
        ...
        try {
            ...
            data.addColumn("image", "CONCAT('/images/',id,'.jpg')");
            JPrefuseTable.showTableWindow(data);

            /*
            edges.addColumn("id", String.class);
            edges.addColumn("image", String.class);

            graph = new Graph(data, edges, true, "id", "image");
            
            for(int i=1; i<=6; i++){
                Node n1 = graph.addNode();
                n1.set("id", data.getColumnNumber(data.getColumn(i)));
                n1.set("image", data.getColumnNumber(data.getColumn(i)));
